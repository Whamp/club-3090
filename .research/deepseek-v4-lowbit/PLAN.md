...
9. The tool now wraps pinned AutoRound's plain symmetric RTN and imatrix-weighted scale-search primitives behind one comparison interface. It returns signed codes, stored FP16 group scales, reconstruction from those persisted values, and weighted and unweighted MSE. Fourteen tests pass in the pinned AutoRound CPU environment; the synthetic W2 fixture improves weighted MSE from about 0.05703 to 0.05031. This verifies the mechanism only. Representative DeepSeek layers still decide whether weighted search is useful.
...
3. **In progress:** the tensor-name classifier, exact size planner, imatrix parser, expert-vector mapping, and plain-versus-weighted RTN comparison are implemented and verified against pinned contracts. Prepare the resumable shard writer and durable output location next; directly validate the parser and quantizer on published/official data while staging the pilot.
