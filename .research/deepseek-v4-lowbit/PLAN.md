...
2. The original W2/W3 Humming bridge `b5a0637b4` applied cleanly as local commit `095557aba`; the equivalent canonical-fork commit is [`1bd04b9a`](https://github.com/Whamp/vllm/commit/1bd04b9ad6a49cf226fc52d0ca4c820f9fc89b21), with the same Git tree.
3. Original local commit `e5a8452c7`, published equivalently as [`c73b07b1`](https://github.com/Whamp/vllm/commit/c73b07b10ef180b75c778be0fa428983c7828bca), adds projection-sensitive compressed-tensors WNA16 MoE support: fused gate/up must share a schema, down may use a different bit width, mixed schemas are Humming-only, and packed shapes and Humming descriptors remain distinct. It also makes the haosdent SM80 FP8 helper import safely when Triton is disabled, so CPU tests can collect without a GPU.
...
16. The exact `humming-kernels==0.1.10` PyPI artifact is a 184,889-byte pure-Python wheel with SHA-256 `4ded0998ff085afeddde70baf93f97c2929969ec3d4a63a52cfec5072bc972b4`, built from `inclusionAI/humming@4351af3a8fcdce1a8dee50104ba49566af2427fb`. It contains JIT CUDA sources but no packaged `.so`, PTX, cubin, or fatbin. Runtime reads the current device capability, maps exact capability 8.6 to dedicated `Sm86Heuristics` with a 99 KiB shared-memory limit and BF16 support, then passes `sm_86` to NVRTC or `compute_86,code=sm_86` to NVCC. Its compressed-tensors schema maps symmetric pack-quantized W2/group-128 to uint2 weights and BF16 scales. These facts prove a specific source/build path, not successful compilation or dispatch. Pinned upstream GPU tests cover dense uint2 and uint4 MoE separately but not uint2 + group-128 + BF16 MoE. Original local vLLM commit `f4d05732a`, published equivalently as [`5b82ae77`](https://github.com/Whamp/vllm/commit/5b82ae778c8915ba1928908d2765c4ff75aa9cf0), adds that exact deterministic indexed-MoE numerical oracle at the real conversion/experts seam; it collects and skips on the CPU host, passes all applicable pre-commit hooks and slop checks, and remains deferred first to rental A100 for generic kernel correctness and then—only after explicit GPU-use authorization—to server60 for generated sm_86 cubin, numerical, and dispatch evidence.
17. The canonical experimental vLLM source is [Whamp/vllm#1](https://github.com/Whamp/vllm/pull/1) on branch `incubate/deepseek-v4-wna16-sm86`. Club-3090 retains a reproducible deployment mirror at `models/deepseek-v4-flash-0731/vllm/patches/deepseek-v4-wna16-sm86/`: eight SHA-256-listed mail patches plus a fail-closed installer. The public branch was reconstructed from those patches, so its commit IDs differ from the original local mail-patch provenance while each production stage preserves the same Git tree. It intentionally excludes the diagnostic-only model-memory-report commit. The canonical branch and mirror both produce final tree `aeb62948e33074514a742d19c2f9a1a3c2ee3e1f`. The installer accepts only clean `haosdent/vllm@12810046c799cbe874967e19b1c0fa134ab7b209` or that exact final tree, uses non-fuzzy `git am`, is idempotent on the final tree, and rejects base drift without modification. A checksum-pinned thin image bakes the seven production files changed by patches 0005–0008 over the verified runtime base, creates the corrected model view at startup, and is wired to the direct-Compose 215K profile. The patch registry records this as verified `runtime_image` delivery. vLLM PR #48918 remains tracked in `docs/UPSTREAM.md`.
