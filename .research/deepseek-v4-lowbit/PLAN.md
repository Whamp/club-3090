...
13. The full rental path is also staged without creating resources. It requires an explicit plain-versus-weighted pilot decision and injected Hugging Face token, resumes the pinned full checkpoint and all-W2 conversion, omits local state from the private upload, then checks the exact remote inventory and every content hash before the rental can be deleted. The upload verifier passed a live read-only adapter check against `hf-internal-testing/tiny-random-gpt2@71034c5d8bde858ff824298bdedc65515b97d2b9`: all 10 files and 12,512,611 bytes matched across three LFS SHA-256 objects and seven ordinary Git blobs.
...
15. A read-only server60 storage preflight on 2026-08-12 found 99 GiB free on the 595 GiB `/mnt/models` filesystem and 115 GiB free on the root filesystem, with ample inodes. Docker already holds 70.76 GB of images and 79.93 GB of build cache; no data was reclaimed. The 76.770 GiB all-W2 artifact fits with about 22 GiB model-volume margin, while the 85.770 GiB anchor is too tight without separately authorized cleanup. Stage exactly one Hub cache copy under `/mnt/models/huggingface/hub`, direct transient `HF_XET_CACHE` data to the root filesystem, and do not create a second local artifact copy. This probe did not query GPUs, call the model, alter containers, or write server60 files.
...
20. The private artifact repository `hampsonw/DeepSeek-V4-Flash-0731-WNA16` now exists and contains only Hub-managed `.gitattributes`. The active shell's `HF_TOKEN` is read-only, while the stored `upload` credential has fine-grained `repo.write` permission. Rental setup must transfer the upload credential without printing it. The full-run preflight validates the exact environment token's namespace write grant and verifies that the target repository is private before downloading the full checkpoint.
21. The selected 350 GB boot NVMe supplies about 325.96 GiB. The pinned source payload is 155.42 GiB and the all-W2 output payload is 76.77 GiB, leaving about 93.77 GiB before the OS, environments, metadata, headers, and transient state. The full-run 260 GiB free-space gate runs against `RENTAL_ROOT`; at that boundary, source plus output still leave about 27.81 GiB. Hugging Face local-directory downloads write payloads directly under the source directory and keep only bookkeeping under its `.cache`, avoiding a second full Hub-cache copy.
...
3. **Done for local conversion tooling and rental definition:** tensor classification, exact sizing, imatrix parsing/mapping, bounded quantizer pilot, official MXFP4/E8M0 dequantization, compressed-tensors packing, streamed source-shard transformation, checksum-bound resume, metrics retention, config/index materialization, and conversion CLIs are implemented against pinned contracts. The exact 24-matrix pilot and idempotent staging script are defined. Live Verda checks selected an on-demand `1A100.22V` with 350 GB boot NVMe and the Ubuntu 24.04 CUDA 13.0 Docker image at $1.8859/hour total; the script's pinned Torch 2.13 cu130 wheel and repository fetches resolve. The two-hour pilot cap is $3.77 and the ten-hour total generation cap is $18.86. The private model repository exists and its write/privacy preflight passes with the stored upload credential. No VM has been created; billable staging remains blocked on the required explicit deployment confirmation.
