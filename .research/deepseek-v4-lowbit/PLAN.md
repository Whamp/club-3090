...
3. **Done for local conversion tooling:** tensor classification, exact sizing, imatrix parsing/mapping, plain-versus-weighted RTN, official MXFP4/E8M0 dequantization, compressed-tensors packing, streamed source-shard transformation, checksum-bound resume, metrics retention, config/index materialization, and the conversion CLI are implemented against pinned contracts. The next step is to define the exact representative-shard pilot, durable rental paths, instance, time/cost cap, and direct published-artifact checks, then stage it on non-interruptible capacity.
