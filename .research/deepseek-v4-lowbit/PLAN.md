...
3. **Done for local conversion tooling:** tensor classification, exact sizing, imatrix parsing/mapping, bounded quantizer pilot, official MXFP4/E8M0 dequantization, compressed-tensors packing, streamed source-shard transformation, checksum-bound resume, metrics retention, config/index materialization, and conversion CLIs are implemented against pinned contracts. The exact 24-matrix pilot is defined. The remaining pre-rental work is to confirm durable paths, instance/image/key, and time/cost cap, then stage it on non-interruptible capacity.
