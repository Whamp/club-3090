...
TYPE = {  # id -> (name, blck_size, type_size)
    0: ("F32", 1, 4), 1: ("F16", 1, 2), 8: ("Q8_0", 32, 34), 9: ("Q8_1", 32, 36),
    10: ("Q2_K", 256, 84), 16: ("IQ2_XXS", 256, 66), 24: ("I8", 1, 1),
    26: ("I32", 1, 4), 27: ("I64", 1, 8), 28: ("F64", 1, 8), 30: ("BF16", 1, 2),
}
VTYPE = {0: "<B", 1: "<b", 2: "<H", 3: "<h", 4: "<I", 5: "<i", 6: "<f", 7: "<?",
         10: "<Q", 11: "<q", 12: "<d"}
...
class R:
    def __init__(self, mm):
        self.mm, self.p = mm, 0
    def u32(self):
        v = struct.unpack_from("<I", self.mm, self.p)[0]; self.p += 4; return v
    def u64(self):
        v = struct.unpack_from("<Q", self.mm, self.p)[0]; self.p += 8; return v
    def raw(self, n):
        v = self.mm[self.p:self.p + n]; self.p += n; return v
    def string(self):
...
def value(r, t):
    ...
    if t == 9:
        ...
        if et not in (0, 1, 2, 3, 4, 5, 6, 7, 10, 11, 12) and et != 8:
            r.raw(n * 8); return [f"<unsupported array etype {et}>"] * min(n, 4)
...
def main(path):
    ...
    for _ in range(n_tensors):
        ...
        tensors.append({"name": name, "type": tname, "dims": dims,
