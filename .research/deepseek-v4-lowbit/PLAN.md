...
3. **In progress:** the tensor-name classifier, exact size planner, imatrix parser, expert-vector mapping, plain-versus-weighted RTN comparison, exact compressed-tensors packing, and resumable checksum-bound shard writer are implemented and verified against pinned contracts. The remaining local tooling seam is the streamed source-shard transform: official MXFP4/E8M0 dequantization, preservation/omission rules, quantization and packing, receipt fingerprints, and config/index materialization. Choose the durable rental output location and directly validate the parser and quantizer on published/official data while staging the pilot.
