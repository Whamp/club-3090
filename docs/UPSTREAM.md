...
|---|---|---|
| **Genesis pin bump `2db18df` → `f2147ad`** (drops `patch_inputs_embeds_optional.py` sidecar; PN35 in Genesis dev backports vllm#35975 natively) | Sandermage typically lands stable tags from dev tip — waiting for him to surface a v7.70 stable tag or "this commit is good" signal before bumping. Dev tip can churn. | (a) Sandermage tags / signals stable; (b) review whether the PN35 + 4 reorg commits soak-tested cross-rig in #19 thread. If clean, bump + drop the sidecar from `long-text.yml` + `long-text-no-mtp.yml`. |
| **Enable P68/P69 across composes** (50K-char threshold now in v7.69 — we're already on it, just have the env vars commented out) | Held in the same Genesis-touching pause. The 50K threshold should clear typical IDE-agent contexts (Cline ~30K / Cursor ~25K / Copilot ~20-25K) while genuine long-history sessions still trigger the reminder. | Genesis pin bump unblocks; or independent decision after a few more days of Sandermage's v7.65+ soak across the community. Affected files: `long-text.yml`, `long-text-no-mtp.yml`, `bounded-thinking.yml`, `dual.yml`, `dual-turbo.yml`, `dual-dflash*.yml`, `tools-text.yml`. |
