...
11. The full source-shard transform and `deepseek-v4-convert` CLI are implemented. They reuse pinned AutoRound's official DeepSeek MXFP4/E8M0 path one routed weight at a time, support plain or imatrix-weighted RTN, preserve non-routed tensors, omit MTP, generate exact projection-specific compressed-tensors metadata, disable MTP in the output config, copy non-weight assets, and finalize only complete 48-shard runs. The captured official headers confirm zero missing or cross-shard weight/scale pairs across all 35,328 routed weights. A combined-toolchain test exercises real FP4 dequantization through finalized MTP-free artifact output; the selected vLLM fork resolves the generated mixed W2/W4 gate/up/down targets as intended.
